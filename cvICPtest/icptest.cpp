...
float flann_knn(Mat& m_destinations, Mat& m_object, vector<int>& ptpairs, vector<float>& dists = vector<float>()) {
	...
	cv::flann::Index flann_index(dest_32f, cv::flann::KDTreeIndexParams(4));  // using 4 randomized kdtrees
...
void Image2PointCloudMat(Mat& img , Mat& dst , int th  )
{
	Mat th_dst;
	vector<Point> point;
	...
	threshold(img, th_dst, th, 255, THRESH_BINARY);

	for (int y = 0; y < th_dst.rows; y+=d)
	{
		for (int x = 0; x < th_dst.cols; x+=d)
		{
			if (th_dst.data[y * th_dst.cols + x] && !isContinuous)
			{
				point.push_back(Point(x, y));
				isContinuous = true;
			}
			else if (!th_dst.data[y * th_dst.cols + x]) isContinuous = false;
		}
	}
	for (int x = 0; x < th_dst.cols; x += d)
	{
...
int main(int argc, char** argv) {
	...
	rng.fill(destinations, RNG::NORMAL, Scalar(150.0), Scalar(125.0));
	...
	*/
	...
	Mat destinations, X;
	Image2PointCloudMat(destinations_img, destinations, 10);
	Image2PointCloudMat(X_img, X, 10);
